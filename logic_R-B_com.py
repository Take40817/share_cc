...
handlerFile = FileHandler(
    "C:/Users/net07/Desktop/CC/conf_log/R-B_Bot.log"
)  # ログファイルの出力先とファイル名を指定
    ...
    while True:
        try:
            ...
            response = requests.get(
                "https://min-api.cryptocompare.com/data/histominute", params, timeout=10
            )
...
def print_price(data):
    logger.info(
        "時間： "
        + datetime.fromtimestamp(data["close_time"]).strftime("%Y/%m/%d %H:%M")
        + " 始値： "
        + str(data["open_price"])
        + " 終値： "
...
def check_candle(data, side):
    if data["high_price"] - data["low_price"] == 0:
        return False
    else:
...
def check_ascend(data, last_data):
    if (
        data["open_price"] > last_data["open_price"]
...
def buy_signal(data, last_data, flag):
    if flag["buy_signal"] == 0 and check_candle(data, "buy"):
        ...
        flag["buy_signal"] == 1
        and check_candle(data, "buy")
        and check_ascend(data, last_data)
    ):
        flag["buy_signal"] = 2
    elif (
        flag["buy_signal"] == 2
...
def sell_signal(data, last_data, flag):
    if flag["sell_signal"] == 0 and check_candle(data, "sell"):
        ...
        flag["sell_signal"] == 1
        and check_candle(data, "sell")
        and check_descend(data, last_data)
    ):
        flag["sell_signal"] = 2
    elif (
        flag["sell_signal"] == 2
    ...
    try:
        for o in orders:
            bitflyer.cancel_order(
                symbol="BTC/JPY", id=o["id"], params={"product_code": "BTC_JPY"}
            )
...
flag = {
    "buy_signal": 0,
    "sell_signal": 0,
    "order": {"exist": False, "side": "", "count": 0},
    "position": {"exist": False, "side": ""},
}
